...
public class FoodListAdapter extends BaseAdapter implements
	...
	public FoodListAdapter(Activity activity, List<Food> mList, TextView order_cart){
		mActivity = activity;
		foods = mList;
		shopCart = order_cart;
		initFoodNum();
...
//	}
	...
	private void initFoodNum() {
		mFoodsNum = new int[foods.size()];
		for (int i = 0; i < mFoodsNum.length; i++) {
			mFoodsNum[i] = 0;
		}
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		holder.add.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				mFoodsNum[position]++;
				if(mFoodsNum[position] < 0){
					mFoodsNum[position] = 0;
				}
				holder.size.setText(mFoodsNum[position] + "");
				if (mFoodsNum[position] > 0) {
					holder.minus.setVisibility(View.VISIBLE);
					shopCart.setText("总计￥" + getTotal());
				}
		...
		holder.add.setText("+ " + food.getPrice() + "元");
